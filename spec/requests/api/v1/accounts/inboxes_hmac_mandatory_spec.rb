...
RSpec.describe 'GET /api/v1/inboxes/:id', type: :request do
  ...
  describe 'hmac_mandatory serialization' do
    ...
    context 'when inbox is WebWidget channel' do
      let(:web_widget_channel) { Channel::WebWidget.create!(hmac_mandatory: true) }
    ...
    context 'when inbox is WhatsApp channel' do
      let(:whatsapp_channel) { Channel::Whatsapp.create!(provider: 'evolution') }
    ...
    context 'when inbox is Email channel' do
      let(:email_channel) { Channel::Email.create!(email: 'test@example.com', forward_to_email: 'forward@example.com') }
