...
RSpec.describe Api::V1::Accounts::ContactsController, type: :controller do
    ...
    context 'when contact deletion fails due to foreign key constraint' do
      ...
      before do
        allow_any_instance_of(Contact).to receive(:destroy!).and_raise(
          ActiveRecord::StatementInvalid.new('PG::ForeignKeyViolation: ERROR')
        )
      end
  ...
  describe 'AC2: Pipeline list endpoints after contact deletion' do
    ...
    it 'does not return orphaned cards in pipeline items list after contact deletion' do
      ...
      json_data = JSON.parse(response.body)
      items = json_data.dig('data') || []
      expect(items).to be_empty
    end

    it 'serializes orphaned items with is_orphaned flag when they exist' do
      pipeline_item = PipelineItem.create!(
        pipeline: pipeline,
        pipeline_stage: pipeline_stage,
        contact: contact,
      ...
      items = json_data.dig('data') || []
      if items.any?
        orphaned_item = items.find { |item| item['contact_id'] == contact_id }
        expect(orphaned_item['is_orphaned']).to be true if orphaned_item
      end
    end
  end
end
